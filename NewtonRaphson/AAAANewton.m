...
L=zeros(N_Element,1);
for i=1:N_Force
    ForceMatrix(Force(i,1)*3-2:Force(i,1)*3,1)=Force(i,2:4)'; 
...
for i=1:N_Boundary
    if Boundary(i,2)==0;
        m=3*Boundary(i,1)-2;
        del=[del,[m]]; %受约束节点位移为0时所对应的指标数值 1
    end
    if Boundary(i,3)==0;
        m=3*Boundary(i,1)-1;
        del=[del,[m]]; %受约束节点位移为0时所对应的指标数值 2
    end
    if Boundary(i,4)==0;
        m=3*Boundary(i,1);
...
Update_Node=Node+Displacement(:,1:2); %更新后的节点坐标向量(x , y 坐标) 
...
while norm(Val)>1e-7 & k<500 %在一个荷载增量步下进行的对此迭代
    ...
    for i=1:N_Element
        ...
        Update_Node=Node+All_Disp(:,1:2)+Correc_dis1(:,1:2);
        %为了求切线刚度矩阵(改) a2下 
        if abs(Update_Node(11,2))<=1e-4&&abs(Update_Node(11,1))<=1e-4
            break
        end
        for i=1:N_Element
